...
assign  pos_tx_en = (~tx_en_d1) & tx_en_d0;         //ARP发送使能信号上升沿
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        //ARP数据   硬件类型（2）、协议类型（2）、硬件地址长度（1）、协议地址长度（1）、操作码（2）、发送端MAC地址（6）、发送端IP地址（4）、接收端MAC地址（6）、接收端IP地址（4）
        ...
        case(next_state)
            st_idle : begin
                if(pos_tx_en) begin
                    skip_en <= 1'b1;  
                    //如果目标MAC地址和IP地址已经更新,则发送正确的地址，不更新则发送默认地址
            ...
            st_preamble : begin                          //发送前导码+帧起始界定符、7+1 Byte
                gmii_tx_en <= 1'b1;
            ...
            st_eth_head : begin                          //发送以太网首部、14 Byte
                gmii_tx_en <= 1'b1;
                crc_en <= 1'b1;
            ...
            st_arp_data : begin                          //发送ARP数据、28 Byte
                crc_en <= 1'b1;
                gmii_tx_en <= 1'b1;
                //至少发送46个字节
                ...
                if(data_cnt <= 6'd27) begin
                    ...
                    gmii_txd <= 8'd0;                    //Padding,填充0、18 Byte
            end
            st_crc      : begin                          //发送CRC校验值、4 Byte
                gmii_tx_en <= 1'b1;
                cnt <= cnt + 1'b1;
                if(cnt == 6'd0)                          //需要反向发送（CRC反向处理）
                    gmii_txd <= {~crc_next[0], ~crc_next[1], ~crc_next[2],~crc_next[3],         //第一个Byte来自crc_next
                                 ~crc_next[4], ~crc_next[5], ~crc_next[6],~crc_next[7]};
                else if(cnt == 6'd1)
                    gmii_txd <= {~crc_data[16], ~crc_data[17], ~crc_data[18],                   //接下来三个Byte来自crc_data

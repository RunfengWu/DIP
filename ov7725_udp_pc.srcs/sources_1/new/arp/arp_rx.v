...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        case(next_state)
            ...
            //前导码（7 Byte）、SFD（1 Byte）、目的MAC地址（6 Byte）、源MAC地址（6 Byte）、以太网协议类型（2 Byte）、数据（46 Byte）、FCS（4 Byte）
            ...
            st_eth_head : begin
                if(gmii_rx_dv) begin
                    cnt <= cnt + 5'b1;
                    if(cnt < 5'd6)                                  //将6Byte目的MAC地址拼接
                        des_mac_t <= {des_mac_t[39:0],gmii_rxd};    //解析目的MAC地址
                    else if(cnt == 5'd6) begin                      //拼接完成
                        //判断MAC地址是否为开发板MAC地址或者公共地址
                        if((des_mac_t != BOARD_MAC)                 //开发板MAC地址
                            && (des_mac_t != 48'hff_ff_ff_ff_ff_ff))//公共地址（上位机MAC地址）
                            error_en <= 1'b1;
                    end
                    else if(cnt == 5'd12) 
                        eth_type[15:8] <= gmii_rxd;          //以太网协议类型（2Byte）
                    else if(cnt == 5'd13) begin
                        eth_type[7:0] <= gmii_rxd;
                        cnt <= 5'd0;
                        if(eth_type[15:8] == ETH_TPYE[15:8]  //判断是否为ARP协议（0x0806）
            ...
            //硬件类型（2）、协议类型（2）、硬件地址长度（1）、协议地址长度（1）、操作码（2）、源MAC地址（6）、源IP地址（4）、目的MAC地址（6）、目的IP地址（4）
            st_arp_data : begin
                if(gmii_rx_dv) begin
                    cnt <= cnt + 5'd1;
                    if(cnt == 5'd6) 
                        ...
                        if(des_ip_t == BOARD_IP) begin       //判断目的IP地址和操作码
                            if((op_data == 16'd1) || (op_data == 16'd2)) begin  //判断请求还是应答
                                skip_en <= 1'b1;
                                arp_rx_done <= 1'b1;         //ARP接收完成
                                src_mac <= src_mac_t;        //寄存源MAC地址
                                src_ip <= src_ip_t;          //寄存源IP地址
                                src_mac_t <= 48'd0;
                                src_ip_t <= 32'd0;
                                des_mac_t <= 48'd0;
                                des_ip_t <= 32'd0;
            ...
            st_rx_end : begin     
                cnt <= 5'd0;
                //单包数据接收完成，gmii_rx_dv为0时，表示数据接收完成

...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        case(next_state)
            st_idle : begin
                if((gmii_rx_dv == 1'b1) && (gmii_rxd == 8'h55))     //输入数据有效，且数据第一位8'h55
                    skip_en <= 1'b1;
            end
            st_preamble : begin
            ...
            st_eth_head : begin                              //解析以太网帧头
                if(gmii_rx_dv) begin
                    ...
                    if(cnt < 5'd6)                           //拼接并寄存目的MAC地址
                       ...
                       && eth_type[15:8] == ETH_TYPE[15:8] && gmii_rxd == ETH_TYPE[7:0])   //eth_type[7:0]延迟一个时钟，此时并未生效，所以用gmii_rxd进行判断         
                            skip_en <= 1'b1;
                        else
                            error_en <= 1'b1;
                    end        
            ...
            st_ip_head : begin                              //解析IP首部
                if(gmii_rx_dv) begin
                    cnt <= cnt + 5'd1;
                    if(cnt == 5'd0)                         //高位为版本号和低位为首部长度
                        ip_head_byte_num <= {gmii_rxd[3:0],2'd0};//IP首部长度*4转换为字节数
            ...
            st_udp_head : begin                              //解析UDP首部（只解析UDP字节长度）
                if(gmii_rx_dv) begin
                    cnt <= cnt + 5'd1;
                    if(cnt == 5'd4)                          //将第五和第六字节拼接成UDP字节长度
            ...
            st_rx_data : begin         
                ...
                if(gmii_rx_dv) begin
                    ...
                    if(data_cnt == data_byte_num - 16'd1) begin //接收数据等于上面解析的数据长度时，接收完成，设置对应参数

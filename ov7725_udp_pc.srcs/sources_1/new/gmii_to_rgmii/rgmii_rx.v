...
assign gmii_rx_dv = gmii_rxdv_t[0] & gmii_rxdv_t[1];          //gmii_rxdv_t[0]为上升沿采集的数据有效信号，gmii_rxdv_t[1]为下降沿采集的有效信号和ERR信号异或的结果，例：rxdv=1，ERR=1，则为 1 & （1 ^ 1）=0
                                                              //rxdv=1，ERR=0，则为 1 & （1 ^ 0）=1，数据有效
...
//*********************************************************************************** 控制信号 ***********************************************************************************//
//输入延时控制
//当例化IDEALY2时，一定要同时例化IDELAYCTRL，否则会出现错误
(* IODELAY_GROUP = "rgmii_rx_delay" *) 
IDELAYCTRL  IDELAYCTRL_inst (
    .RDY(),                      // 1-bit output: Ready output
    .REFCLK(idelay_clk),         // 1-bit input: Reference clock input，PLL输出200Mhz
...
IDELAYE2 #(
  ...
  .IDELAY_VALUE    (IDELAY_VALUE),      // Input delay tap setting (0-31)，输入延时值为0
...
//输入双沿采样寄存器，将多bit单沿数据转换为双沿多bit单沿数据
IDDR #(
